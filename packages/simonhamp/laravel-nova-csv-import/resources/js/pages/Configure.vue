<template>
    <div>
        ...
        <!-- Adding a card for Contact List selection -->
        <card class="p-8 space-y-4 mb-8" v-if="resource === 'contacts'">  <!-- assuming 'Contact' is the resource name for the contacts -->
            <p>
                Choose a Contact List to attach the contacts. 
            </p>

            <div class="inline-flex items-center">
                <b>Contact List:</b>
                <SelectControl @change="(value) => { contactListId = value; console.log('Selected contactListId: ', contactListId); }" :selected="contactListId" class="ml-4">
    <option value="">- Select a Contact List -</option>
    <option v-for="(contactList, index) in contactLists" :value="contactList.id">{{ contactList.name }}</option>
</SelectControl>
            </div>
        </card>
