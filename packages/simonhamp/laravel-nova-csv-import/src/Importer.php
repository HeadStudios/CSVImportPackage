    ...
    public function model(array $row): Model
{
    ...
    if($model instanceof \App\Models\Contact) {
            ...
            // Check user's role
            if(Auth::user()->hasRole('sales-kid') || Auth::user()->hasRole('super-admin')) {
                $model->owner_id = Auth::id();
            } elseif (Auth::user()->hasRole('importer')) {
                $model->imported_by = Auth::id();
            }
            ...
            if ($listId) {
                ...
                // Make sure the ContactList exists
                if($contactList) {
                    // Connect the current contact to the ContactList
                    $model->lists()->attach($contactList);
                }
            }
    }
